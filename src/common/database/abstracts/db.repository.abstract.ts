...
import { DatabaseSoftDeleteDto } from '@/common/database/dtos/db.soft-delete.dto';
...
> {
  ...
  async findAll<T = EntityDocument>(
    ...
    if (options?.paging) {
      repository
        ...
        .skip(options.paging.pageSize * (options.paging.pageNumber - 1));
    }
    ...
    const data = await repository.exec();
    const totalItems = data.length;

    return new PaginatedResult(
      data,
      {
        pageSize: options?.paging?.pageSize ?? 10,
  ...
  >(
    ...
    if (options?.paging) {
      newPipelines.push(
        {
          $limit: options.paging.pageSize + options.paging.pageNumber,
        },
        { $skip: options.paging.pageNumber },
      );
    }
